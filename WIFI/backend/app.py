...
from modules import ContainerMonitor, StreamMonitor, WiFiManager
...
def health_check():
    return jsonify({"status": "ok", "service": "orin-system-monitor"})
...
def get_streams_status():
    try:
        ...
        return jsonify(
            {"success": True, "data": {"container_running": True, "streams": streams}}
        )
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
