...
class StreamMonitor:
    ...
    def get_supervisord_status(self) -> Dict:
        ...
        try:
            ...
            if (
                ...
                and "FATAL" not in result.stdout
            ):
                return {
            ...
            for line in result.stdout.strip().split("\n"):
                ...
                if len(parts) >= 2:
                    ...
                    processes[name] = {
                        ...
                        "pid": (
                            parts[2] if len(parts) > 2 and parts[2].isdigit() else None
                        ),
                    }

            if not processes:
                return {
                    "error": "No supervisor processes found",
        ...
        try:
            ...
            for line in result.stdout.split("\n"):
                if "ffmpeg" in line.lower() and stream_name in line.lower():
                    return {"running": True, "process_found": True}

            return {"running": False, "process_found": False}
    ...
    def check_camera_device(self, device_path: str = "/dev/video0") -> Dict:
        ...
        try:
            ...
            return {"accessible": False, "device": device_path, "error": str(e)}
    ...
    def check_audio_device(self) -> Dict:
        ...
        try:
            ...
            if result.returncode == 0:
                return {"accessible": True, "pulseaudio_connected": True}
            else:
                return {
                    "accessible": False,
            ...
            if supervisor_prog in supervisor_status:
                ...
                if stream_info.get("cloud_stream"):
                    ...
                    if stream_info["type"] == "video" and "device" in stream_info:
                        ...
                        base_status["device_accessible"] = device_status.get(
                            "accessible", False
                        )

                    if stream_info["type"] == "audio":
                        audio_status = self.check_audio_device()
                        base_status["device_accessible"] = audio_status.get(
                            "accessible", False
                        )

                    if prog_status["running"] and "rtsp_path" in stream_info:
                        rtsp_status = self.check_local_rtsp_stream(
                            stream_info["rtsp_path"]
                        )
                        base_status["rtsp_streaming"] = rtsp_status.get(
                            "streaming", False
                        )
                        if not rtsp_status.get("streaming"):
                            # If process running but stream not available
                            base_status["status"] = "running_no_stream"
                            base_status["stream_error"] = rtsp_status.get(
    ...
    def get_container_status(self) -> Dict:
        ...
        try:
            ...
            if result.returncode == 0 and result.stdout.strip():
                ...
                return {"running": False, "status": "Container not running"}
