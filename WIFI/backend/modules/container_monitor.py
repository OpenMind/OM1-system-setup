...
class ContainerMonitor:
    ...
    def get_container_status(self, container_name: str) -> Dict:
        ...
        try:
            ...
            return {"running": running.lower() == "true", "status": status}
    ...
    def get_ros2_camera_streams(self) -> Dict:
        ...
        for camera_name in ["front_camera", "down_camera"]:
            ...
            if self.stream_monitor:
                ...
                stream_info["status"] = (
                    "running" if rtsp_status.get("streaming") else "stopped"
                )
    ...
    def get_all_containers_status(self) -> Dict:
        ...
        if self.stream_monitor:
            ...
            result["video_processor"]["container_status"] = {
                ...
                "status": (
                    "running" if container_running_status.get("running") else "stopped"
                ),
            }
            all_streams = self.stream_monitor.get_all_streams_status()
        else:
            result["video_processor"]["container_status"] = self.get_container_status(
                "om1_video_processor"
            )
            all_streams = {}
                ...
                result["video_processor"]["cloud_streams"].append(
                    {
                        ...
                        "status": all_streams["top_camera_cloud"].get(
                            "status", "unknown"
                        ),
                        "type": "video",
                    }
        ...
        # ROS2 Sensor
        result["ros2_sensor"]["container_status"] = self.get_container_status(
            "om1_sensor"
        )
        ...
        # Orchestrator
        result["orchestrator"]["container_status"] = self.get_container_status(
            "orchestrator"
        )
