...
OTA_SERVER_URL = os.getenv(
    "OTA_SERVER_URL", "wss://api.openmind.org/api/v1/ota/updater"
)
...
def apply_ota_update(
    service_name: str, yaml_content: dict, temp_yaml_path: str, tag: str, ws_client=None
):
    """
    ...
    send_progress_update(ws_client, "starting", f"Starting OTA update {tag}", 0)
    ...
    try:
        ...
        stop_result = stop_docker_services(yaml_content)
        if not stop_result.get("success"):
            error_msg = f"Failed to stop Docker services: {stop_result.get('error', 'Unknown error')}"
            logging.error(error_msg)
            send_progress_update(ws_client, "error", error_msg, 10)

        send_progress_update(ws_client, "storing", "Storing update files", 20)
        ...
        try:
            ...
            logging.error(error_msg)
            send_progress_update(ws_client, "error", error_msg, 20)
            return False

        logging.info("Starting updated Docker services...")
        start_result = start_docker_services(yaml_content, ws_client)
        if not start_result.get("success"):
            error_msg = f"Failed to start Docker services: {start_result.get('error', 'Unknown error')}"
            logging.error(error_msg)
            send_progress_update(ws_client, "error", error_msg, 80)
            return False
        ...
        logging.error(error_msg)
        send_progress_update(ws_client, "error", error_msg, 0)
        return False


def stop_docker_services(yaml_content: dict) -> dict:
    """
...
def pull_images_with_progress(pull_cmd: list, ws_client=None) -> dict:
    """
            ...
            if output:
                ...
                if "Pulling" in line:
                    ...
                    send_progress_update(
                        ...
                        30 + (len(services_pulled) * 40 // max(len(services_found), 1)),
                    )
...
def start_docker_services(yaml_content: dict, ws_client=None) -> dict:
    """
    ...
    try:
        ...
        try:
            temp_compose_file = create_temp_compose_file(yaml_content)
            ...
            pull_result = pull_images_with_progress(pull_cmd, ws_client)
            ...
            up_cmd = ["docker-compose", "-f", temp_compose_file, "up", "-d"]
            ...
            if up_result.returncode == 0:
                ...
                cleanup_result = cleanup_old_images(ws_client)
            ...
            if temp_compose_file and os.path.exists(temp_compose_file):
                try:
                    ...
                    logging.warning(f"Failed to clean up temporary compose file: {e}")
...
def create_temp_compose_file(yaml_content: dict) -> str:
    """
...
def cleanup_old_images(ws_client=None) -> dict:
    """
    ...
    try:
        ...
        send_progress_update(ws_client, "cleanup", "Cleaning up old Docker images", 90)
        ...
        for cmd_name, cmd in zip(["images", "containers", "system"], cleanup_cmds):
            try:
                ...
                if result.returncode == 0:
                    ...
                    cleanup_results.append(
                        {"command": cmd_name, "success": False, "error": result.stderr}
                    )
                    logging.warning(f"Failed to cleanup {cmd_name}: {result.stderr}")

            except subprocess.TimeoutExpired:
                cleanup_results.append(
                    {"command": cmd_name, "success": False, "error": "Cleanup timeout"}
                )
        ...
        if successful_cleanups:
            message = f"Cleaned up Docker resources. Space freed: {total_space_freed}"
            logging.info(message)
            send_progress_update(ws_client, "cleanup_complete", message, 95)
            return {
                "success": True,
                "message": message,
        ...
        logging.error(error_msg)
        send_progress_update(ws_client, "cleanup_error", error_msg, 90)
        return {"success": False, "error": error_msg}


def send_progress_update(ws_client, status: str, message: str, progress: int):
    """
    ...
    if ws_client:
        ...
        try:
            ...
            logging.info(f"Sent progress update: {status} - {message} ({progress}%)")
        except Exception as e:
            logging.warning(f"Failed to send progress update: {e}")


def ota_updater_process(message: str, ws_client=None):
    """
    Process OTA update messages received from the WebSocket server.
        ...
        try:
            ...
            if yaml_content and local_file_path:
                ...
                try:
                    ...
                    logging.warning(f"Failed to clean up file {local_file_path}: {e}")
            else:
...
def create_ws_client() -> WebSocketClient:
    ...
    return WebSocketClient(
        url=f"{OTA_SERVER_URL}?api_key_id={OM_API_KEY_ID}&api_key={OM_API_KEY}"
    )
...
def main():
    ...
    try:
        ...
        def callback_with_client(message):
            return ota_updater_process(message, ws_client)

        ws_client.register_message_callback(callback_with_client)
        ws_client.connect()
    except Exception as e:
